...
See Problem Definition document

#include <bits/stdc++.h>

void exhaust(vector<string> &zombies, int rowStart,
			 set<int> &seen) {

	seen.insert(rowStart);

	for (int col = 0; col < zombies[0].length(); col++) {
		if (zombies[rowStart][col] == '1' &&
			(seen.find(col) == seen.end())) {
			exhaust(zombies, col, seen);
		}
	}
